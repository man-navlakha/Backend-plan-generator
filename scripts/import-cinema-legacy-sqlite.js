...
const workbookPath = path.resolve(process.argv[2] || path.join(__dirname, '../src/assets/Masters/Cinema/cinema.xlsx'));
const imageDirectory = path.join(path.dirname(workbookPath), 'Images');
const COLUMNS = { Product: 19, Location: 9, 'Price Option': 18, 'Price Unit': 11,
...
/**
 * An Indian PIN code is six digits and never starts with a zero. The sheet asks
 * for a number but arrives as text, as a float, or padded, so normalise here and
 * drop anything that is not a PIN -- a half-typed code in the plan reads as fact.
 */
function pincode(input) {
  const digits = String(input ?? '').replace(/[^0-9]/g, '');
  return /^[1-9][0-9]{5}$/.test(digits) ? Number(digits) : null;
}

function forRows(sheet, start, callback, required = true) {
  if (!sheet) {
    if (required) throw new Error('Workbook is missing a required sheet');
    return;
  }
...
async function main() {
  ...
  db.transaction(() => {
    ...
    const addLocation = db.prepare(`INSERT INTO locations
      (product_id,source_row,type,zone,state,city,locality,pincode) VALUES (?,?,?,?,?,?,?,?)`);
    counts.locations = 0;
    forRows(workbook.getWorksheet('Location'), 5, (r, sourceRow) => {
      ...
      addLocation.run(unique(productBySku, text(r[2])), sourceRow, text(r[3]), text(r[4]),
        text(r[5]), text(r[6]), text(r[7]), pincode(r[8]));
      counts.locations += 1;
    });
...
function audit(images) {
  ...
  for (const p of db.prepare('SELECT * FROM products').all()) {
    ...
    if (loc && loc.pincode == null) issue('warning', 'MISSING_PINCODE', 'Location.PIN Code', null, '<6-digit PIN>',
      'PIN Code is blank, so the Pincode column on the client plan renders empty.');
