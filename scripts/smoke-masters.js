...
(async () => {
  ...
  try {
    ...
    assert.equal(cinemaAnnex.data.length, 0, 'new Cinema master should not expose removed annex sheets');
    assert.equal(catalogs.find((item) => item.slug === 'cinema').sheet_counts['Qube Rate Card'], 0);
